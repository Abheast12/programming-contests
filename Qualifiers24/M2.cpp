    ...
    for(int i=0;i<t; i++){
        ...
        for(int j=0; j<n; j++){
            ...
            while(left<=right){
                ...
                long long mmid = end + mid;
                mmid%=n;
                // cout << mid2 << endl;
                long long lmid = (mmid-1+n)%n;
                long long rmid = (mmid+1)%n;

                long long suml1 = suml+v[mmid].first*v[start].second+v[end].first*v[mmid].second;
                long long sumr1 = sumr+v[mmid].second*v[start].first+v[end].second*v[mmid].first;
