...
int main(){
    ...
    for(int i=0; i<n; i++){
        if(v1[i].first!=mina || v1[i].second!=minb){
            if(mina == v1[i].first){
                ...
                long double d = atan2(((long double)(v1[i].second-minb)), ((long double)(v1[i].first-mina)));
    ...
    // v2.push_back({v[1].second.first, v[1].second.second});
    ...
    for(int i=0; i<v2.size(); i++){
        if(i!=v2.size()-1){
            ans+=((long long)(v2[i].first*v2[i+1].second)-(long long)(v2[i+1].first*v2[i].second));
        }
        else{
            ans+=((long long)(v2[v2.size()-1].first*v2[0].second)-(long long)(v2[0].first*v2[v2.size()-1].second));
        }
